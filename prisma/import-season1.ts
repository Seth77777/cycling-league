...
// S1 name -> current (S4) team name, for teams renamed since (same aliases as the S2/S3 imports).
...
async function main() {
  // This file's rows are Nom/Prénom/Pays/Âge/POT/13 stats/MOY/Team — no blank column
  // between Prénom and Pays, unlike the S2/S3 source files.
  const filePath = path.resolve(process.cwd(), "prisma/data/season1-riders.tsv");
  ...
  for (const line of lines) {
    ...
    // No later-season counterpart — this rider's career ends at S1 in our records.
  ...
  console.log(`New S1-only riders created (marked retired, retirementSeason=${SEASON}): ${newRiders.length}`);
